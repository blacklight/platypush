...
class BaseNotePlugin(  # pylint: disable=too-many-ancestors
    ...
    def __init__(
        ...
        *args,
        ...
        sync_from: Optional[List[Union[dict, SyncConfig]]] = None,
        **kwargs,
    ):
        """
        ...
        :param sync_from: A list of dictionaries or SyncConfig instances
            specifying the plugins to sync from. If set, the plugin will subscribe
            to the events generated by the specified plugins and will try to pull
            their notes and collections. Supported keys are:

                - ``plugin`` (``str``): The name of the plugin to sync from.
                - ``pull_remote`` (``bool``): If set to True (default), the
                  plugin will attempt to pull all the notes from the remote
                  upon initialization. If set to False, the plugin will only
                  process changes received while it is running.
                - ``conflict_resolution`` (``SyncConflictResolution``): The
                  strategy to apply when a conflict is detected. A conflict
                  is defined like a case where the remote note is older than
                  its locally synchronized copy, and the changes to the local
                  copy have been performed after the last synchronization.
                  Allowed values are:

                      - ``merge`` (default): Attempt to overwrite the local
                        version of the note with the remote version if the
                        remote version is newer. Otherwise, create a conflict
                        note with the remote version while keeping the local
                        version intact.
                      - ``overwrite``: Always overwrite the local version of
                        the note with the remote version.
                      - ``ignore``: Ignore the conflict and do not update the
                        local note.

                - ``timeout`` (``float``): The timeout in seconds for
                  synchronization operations. If None, no timeout is applied.
                  Defaults to 30 seconds.

                - ``sync_remote_deletions`` (``bool``): If set to True (default),
                  deletions performed on the remote side will be synchronized
                  to the local side, when the local note isn't synced to any
                  other remotes. You may want to disable this if you use
                  multiple mutually connected plugins in order to minimize
                  accidental data loss. It is otherwise recommented in those
                  cases to synchronize the notes also to a source that has
                  this flag set to False as a backup.

                - ``failsafe_delete_threshold`` (``float``): The maximum fraction
                  of notes that can be deleted during a single synchronization
                  operation, as a fraction between 0 and 1. This is a failsafe
                  mechanism to prevent accidental mass deletions. If the
                  fraction of notes that would be deleted exceeds this
                  threshold, the synchronization will be aborted. Set to 0 to
                  disable this failsafe mechanism. The default is 0.25 (25%).
